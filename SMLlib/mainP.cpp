...
int mainP()
{
	...
	grid.createGrid(1, { 10,1 }, { &afP::sign , &afP::linear,afP::tanh,&afP::linear,&afP::linear });
	grid.setEta(0.051);
	...
	for (int show = 0; show < 3000; show++) 
	{
		...
		for (int sample = 0; sample < inputs_vec.size(); sample++)
		{
			...
			{
				grid.calcOutput();
				grid.correctWeightsOneByOne({ refFunct(input)});
			}
		...
		for (int i = 0; i < inputs_vec.size(); i++)
		{
			...
			grid.setInput({ input });
			grid.calcOutput();
			eps += abs(refFunct(input) - (*grid.getOutput())[0]);
	...
	for ( int i=0; i<inputs_vec.size(); i++ )
	{
		...
		refFile << input << "\t" << refFunct(input) << "\t" << (*grid.getOutput())[0] << std::endl;
		std::cout << abs(refFunct(input) - (*grid.getOutput())[0]) << " ";
